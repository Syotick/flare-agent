...
| 2026-08-31 | 🔧 **布局自适应修复（文字一多不超屏）**：`.markdown-body` 加 `overflow-wrap:break-word + word-break:break-word + min-width:0 + max-width:100%`（长 URL/长单词/长文本段落自动换行，根因）；代码块/表格容器 `max-w-full`（内部各自横向滚动）；DirectoryPicker `max-h-[92vh]`（屏幕矮时自适应）+ footer flex-wrap；Composer 权限/模型下拉改 `bottom-full` 向上展开（防底部超屏）；StatusLine/ApprovalCard 描述加 break-words。tsc+build 全过，服务已提供新 dist |
| 2026-08-31 | 📚 **教学文档体系重建（高 star 教学项目风格，有基础的人能看懂）**：README 重写为真实可跑（定位/特性/快速开始/架构图 mermaid/目录/导航/路线图，区分已交付 vs 规划）；新建 **docs/guides/** 6 篇真实教学文档（00 概览 / 01 快速开始 / 02 核心概念 / 03 架构详解 / 04 二次开发 / 05 进阶）+ guides/README；docs/README 加「新手上路」主入口 + learning 标注为演进/规划参考（与代码有差异）；文档全部基于当前真实代码（265 测试、sqlite、六工具、审批、权限模式）。**约束：改 docs 影响方向必须同步 CLAUDE.md** |
| 2026-08-31 | 🔍 **大厂 Agent 开发岗 JD/面试调研（网络实查）**：写 `docs/learning/00-agent-engineering-job-research.md`——真实 JD 五层要求拆解（后端基础/Agent 技术/RAG/Prompt+Eval/模型多模态）、1798 题题库领域分布、开发岗专项（系统设计15/工程实践12/框架10/业务8）、Top10 高频真题、Flare 项目对照表（能当答案的 14 项 vs 需补的 8 项）、2-4 周行动清单。关键发现：Agent 开发岗=后端50%+Agent30%+可观测20%；Eval 评测与中间件（Redis/MQ/K8s/向量库）是 JD 明确要求 |
