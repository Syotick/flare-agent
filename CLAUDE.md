...
- **📚 里程碑现状（截至当前）**：M2 核心循环✅ → M3a 知识库✅ → M3b 分层记忆✅ → M3c RAG评测✅ → M4 模型网关+沙箱✅ → M5 云原生代码层(多租户/任务存储/PgVector/checkpoint/reconcile/otel/infra-k8s)✅ → Web 控制台(对话+知识库+记忆三工作区)✅ → **待办：M6 生产运营(SLO/告警/压测/扩缩容/回滚演练) + 服务器到位后的云部署(learning/05 §4.1)**。教学文档 learning/01–11 齐（11=Web 控制台与产品化 UX），索引在 docs/README.md；CHANGELOG.md 已全量沉淀 M1–M5+Web+UX 条目。
