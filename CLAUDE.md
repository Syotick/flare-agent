...
- 当前阶段：**M2 完成核心闭环**；**M3a RAG 知识库已开工 ✅**（services/rag：切块/嵌入/向量库分层 + KnowledgeBase 门面 + kb_search 工具 + /v1/kb API 入库/列表/检索/删除 + test_rag 10 例 + scripts/demo_rag.py + learning/02 教学文档；pytest 42 全绿，ruff/black 干净）。下一步：M3b 分层记忆（会话短期→项目长期→向量记忆）+ M3c RAG 评测（RAGAS/混合检索/重排）
