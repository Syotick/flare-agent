...
- **📚 里程碑现状（截至当前）**：M2 核心循环✅ → M3a 知识库✅ → M3b 分层记忆✅ → M3c RAG评测✅ → M4 模型网关+沙箱✅ → M5 云原生代码层✅ → M6 生产运营(SLO/告警/压测/门禁/回滚)✅ → Web 控制台(对话+知识库+记忆)✅ → **待办：服务器到位后的云部署(learning/05 §4.1 + 12 §7 回滚演练) + 压测实测容量**。教学文档 learning/01–12 齐（12=生产运营 SRE），索引在 docs/README.md；CHANGELOG.md 已全量沉淀 M1–M6+Web+UX。
